...
func Unmarshal(curve elliptic.Curve, data []byte) (x, y *big.Int) {
	...
	switch params.Name {
	case "SM2-P-256":
		y2 = y2gmsm2(params, tx)
	case "SECP256K1":
		y2 = y2secp256k1(params, tx)
	default:
		y2 = y2ecdsa(params, tx)
	}
	sqrt := defaultSqrt
	ty := sqrt(y2, params.P)
	if ty == nil {
...
func y2ecdsa(curve *elliptic.CurveParams, x *big.Int) *big.Int {
...
// y² = x³ + b
func y2secp256k1(curve *elliptic.CurveParams, x *big.Int) *big.Int {
	x3 := new(big.Int).Mul(x, x)
	x3.Mul(x3, x)
...
// y² = x³ + ax + b
func y2gmsm2(curve *elliptic.CurveParams, x *big.Int) *big.Int {
	x3 := new(big.Int).Mul(x, x)
	x3.Mul(x3, x)
